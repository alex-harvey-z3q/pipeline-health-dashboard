...
function provenance(item) {
  ...
  return `<p class="provenance"><strong>Explicit provenance</strong><br>Execution pool: ${escapeHtml(item.agent_pool || "Unknown")}<br>Image: ${escapeHtml(item.provenance.image_version || "not supplied")}<br>Image build: ${image ? escapeHtml(`${image.project} / definition #${image.pipeline_definition_id} / run #${image.run_id}${image.version ? ` (${image.version})` : ""}`) : "not supplied"}</p>`;
}
...
function render(data) {
  ...
  renderRows($("#smoke-list"), smokes, (item) => `<article class="row"><h3>${escapeHtml(item.pipeline.project)} / ${escapeHtml(item.pipeline.name)} <span class="status ${statusClass(result(item))}">${escapeHtml(result(item))}</span></h3><p>Run: ${link(item.run_url, item.run_number || "No run")} | Tests: ${escapeHtml(tests(item))} | Agent pool: ${escapeHtml(item.agent_pool || "Unknown")}</p>${provenance(item)}</article>`);
  renderRows($("#deployment-list"), data.deployments, (deployment) => `<article class="row"><h3>${escapeHtml(deployment.environment)} <small>${escapeHtml(deployment.deployment_id)}</small></h3><p>Agent pool: ${escapeHtml(deployment.agent_pool || "not supplied")} | Image: ${escapeHtml(deployment.image_version || "not supplied")} | Deployed: ${escapeHtml(deployment.deployed_at || "not supplied")}</p><p>Image build: ${deployment.image_build ? escapeHtml(`${deployment.image_build.project} / definition #${deployment.image_build.pipeline_definition_id} / run #${deployment.image_build.run_id}`) : "not supplied"}</p>${deployment.smoke_tests.map((item) => `<p>Smoke: ${escapeHtml(item.pipeline.name)} - ${escapeHtml(result(item))} (${escapeHtml(tests(item))})</p>`).join("") || "<p>No explicitly associated smoke tests.</p>"}</article>`);

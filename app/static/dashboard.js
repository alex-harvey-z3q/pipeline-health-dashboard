...
function provenance(item) {
  ...
  return `<p class="provenance"><strong>Explicit provenance</strong><br>Agent pool: ${escapeHtml(item.provenance.agent_pool || item.reported_agent_pool || "not supplied")}<br>Image: ${escapeHtml(item.provenance.image_version || "not supplied")}<br>Image build: ${image ? escapeHtml(`${image.project} / definition #${image.pipeline_definition_id} / run #${image.run_id}${image.version ? ` (${image.version})` : ""}`) : "not supplied"}</p>`;
}
